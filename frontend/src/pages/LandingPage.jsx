...
import { Lightning, Plus, Sparkle, Trash, ArrowRight, GithubLogo, SignOut } from "@phosphor-icons/react";
...
export default function LandingPage() {
  ...
  useEffect(() => {
    Promise.all([apiClient.get("/projects"), apiClient.get("/templates")])
      .then(([p, t]) => {
        setProjects(p.data);
        setTemplates(t.data);
      })
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-12 md:col-span-8">
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              ...
              <a href="#templates" className="text-[#F5F5F5] border border-[#2A2A2A] hover:border-[#F5F5F5] font-bold uppercase text-sm tracking-wide px-7 py-4">
                Browse Templates
              </a>
            </div>
          </div>
      ...
      <section id="templates" className="max-w-7xl mx-auto px-6 md:px-8 lg:px-12 py-16 border-b border-[#2A2A2A]">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="font-heading text-2xl sm:text-3xl lg:text-4xl tracking-tight font-bold">Start from a template</h2>
            <p className="mt-3 text-[#A1A1AA] max-w-lg">Pre-built starting points. Pick one, customise via chat.</p>
          </div>
          ...
          {templates.map((tpl) => (
            ...
            >
              <div className="aspect-video overflow-hidden border-b border-[#2A2A2A] bg-[#0D0D0D] relative">
                ...
                <div className="absolute top-3 left-3 bg-[#050505] border border-[#2A2A2A] px-2 py-1 font-mono text-[10px] uppercase tracking-[0.2em]">{tpl.category}</div>
              </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 lg:px-12 py-16">
        ...
        {loading ? (
          <div className="font-mono text-sm text-[#A1A1AA] flex items-center gap-3">
            <span className="ascii-pulse">▓▓▓▓▓░░░░░</span><span>loading projects...</span>
          </div>
        ) : projects.length === 0 ? (
              ...
              >
                ...
                <div className="mt-auto pt-4 flex items-center justify-between">
                  <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#A1A1AA]">
                    {p.current_code ? "● ready" : "○ empty"}{p.files?.length ? ` · ${p.files.length} files` : ""}
                  </span>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={() => setShowNew(false)}>
          <div className="bg-[#121212] border border-[#2A2A2A] w-full max-w-md p-8 slide-up" onClick={(e) => e.stopPropagation()} data-testid="new-project-modal">
            ...
            <div className="flex gap-3">
              <button data-testid="cancel-new-project-btn" onClick={() => setShowNew(false)} className="flex-1 bg-transparent border border-[#2A2A2A] text-[#F5F5F5] hover:border-[#F5F5F5] font-bold uppercase text-xs tracking-wide px-5 py-3">
                Cancel
              </button>
